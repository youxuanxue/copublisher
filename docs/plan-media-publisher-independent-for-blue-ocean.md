# copublisher 独立仓库化与 blue-ocean 调度接入方案

## 1. 结论（先回答你的核心问题）

`copublisher` 可以作为独立仓库、独立工具包，被 `/Users/xuejiao/Codes/blue-ocean` 的「1 人公司 + 多 Agent + 飞书」组织结构调度；并且建议优先走 **“独立包 + 命令行契约 + blue-ocean 适配层”** 路径，而不是在 `blue-ocean` 内重复实现发布逻辑。
...
- **导出层冲突**：`copublisher/__init__.py` 与 `core/__init__.py` 都是重度 eager import。
...
将 `copublisher` 明确为三层能力：

1. `copublisher.domain`（任务模型、校验、平台抽象）
2. `copublisher.application`（发布用例、重试、幂等键、结果模型）
3. `copublisher.interfaces`（CLI/GUI/未来 RPC）

blue-ocean 只依赖最外层稳定接口，不直接触碰 Playwright/API 细节。

建议在 blue-ocean 中新增一个薄适配器（例如 `blue_ocean/org/copublisher_adapter.py`）：

- 入参：`action_id`、平台、素材路径、账号标识、幂等键
- 调用：子进程执行 `copublisher job run ... --json`
...
- `python -c "import copublisher"` 不触发环境变量修改、网络请求或浏览器启动。
...
- 新增命令：`copublisher job run --job-file <json> --result-file <json>`
- 约定结果 schema：
...
- 在审批通过后的执行节点调用 `copublisher_adapter`。
...
### 9.1 文件级改造清单（copublisher）

- `src/copublisher/core/youtube.py`：移除导入时 `_setup_proxy()`，改为显式调用。
- `src/copublisher/__init__.py`：删减 eager import，只保留版本与轻量导出。
- `src/copublisher/core/__init__.py`：改为轻量导出或延迟导入。
- `src/copublisher/core/wechat.py`、`core/browser.py`：账号标识校验与安全路径拼接。
- `src/copublisher/__main__.py`：新增 `job run`、`--json`、结构化退出码。
- `src/copublisher/shared/io.py`（新建）：原子写工具与统一 JSON 写入。
...
1. 在 `copublisher` 完成“无副作用 import + 安全输入 + 原子写 + JSON 契约”。
2. 在 `blue-ocean` 增加 `copublisher_adapter`，打通飞书审批后的执行调用。
